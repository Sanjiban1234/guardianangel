...
import {
  GuardianAngelTTSService,
  TTSPriority,
  TTS_SETTINGS_KEY,
} from '../GuardianAngelTTS';

jest.mock('@mhpdev/react-native-speech');

const speech = Speech as jest.Mocked<typeof Speech>;
const voices = [
  { identifier: 'voice-one', name: 'Voice One', language: 'en-US', quality: 'Default' as const },
];
...
describe('GuardianAngelTTSService', () => {
  ...
  it('clamps invalid speech rates to the supported range', async () => {
    ...
    expect(service.getSettings().rate).toBe(1.5);
    await service.setRate(0.1);
    expect(service.getSettings().rate).toBe(0.5);
  });
  ...
  it('speaks once per separation transition and allows a new separation after reunion', async () => {
    ...
    expect(speech.speak).toHaveBeenLastCalledWith('Ram is 620 meters away from the group.', expect.any(Object));
  ...
  it('does not call native speech while disabled', async () => {
    const service = new GuardianAngelTTSService();
    await service.setEnabled(false);
    jest.clearAllMocks();
