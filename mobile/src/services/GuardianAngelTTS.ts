...
export class GuardianAngelTTSService {
  ...
  async setVoice(voiceId: string | null): Promise<void> {
    ...
    this.settings = {
      ...this.settings,
      voiceId: voiceId && voices.some(voice => voice.identifier === voiceId) ? voiceId : null,
    };
    this.configureNative();
    await this.persist();
  }
  ...
  async getVoices(): Promise<VoiceProps[]> {
    try {
      return (await Speech.getAvailableVoices()).filter(voice => Boolean(voice.identifier && voice.language));
    } catch (error) {
      console.warn('[TTS] voices unavailable', error);
      return [];
    }
  }
  ...
  async stop(): Promise<void> {
    try { await Speech.stop(); } catch (error) { console.warn('[TTS] stop failed', error); }
  }

  async isSpeaking(): Promise<boolean> {
    try { return await Speech.isSpeaking(); } catch { return false; }
  }
  ...
  announceRideEnded(groupCode?: string) {
    return this.speak('Ride ended.', { key: `ride-ended:${groupCode || 'current'}` });
  }
  ...
  announceSeparation(riderId: string, riderName: string, distanceMeters?: number, isCurrentRider = false) {
    ...
    const message = isCurrentRider ? 'You are separating from the group.' : `${riderName}${distance}.`;
    return this.speak(message, { priority: TTSPriority.WARNING, key });
  }
  ...
  private nativeOptions() {
    return { rate: this.settings.rate, ...(this.settings.voiceId ? { voice: this.settings.voiceId } : {}) };
  }

  private configureNative(): void {
    try { Speech.configure(this.nativeOptions()); } catch (error) { console.warn('[TTS] configuration failed', error); }
  }
